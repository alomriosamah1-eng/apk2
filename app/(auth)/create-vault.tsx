...
function getPinStrength(pin: string): { label: string; color: string; percentage: number } {
  if (pin.length <= 4) return { label: 'ضعيف', color: '#EF5350', percentage: 25 };
  if (pin.length <= 6) return { label: 'متوسط', color: '#FFB74D', percentage: 60 };
  return { label: 'قوي', color: '#66BB6A', percentage: 100 };
}
...
export default function CreateVaultScreen() {
  ...
  const strengthInfo = useMemo(() => getPinStrength(pin), [pin]);
  ...
  const handleCreate = useCallback(async () => {
    ...
    if (!name.trim()) { setError('الرجاء إدخال اسم الخزنة'); return; }
    if (pin.length < 4) { setError('الرمز يجب أن يكون 4 أرقام على الأقل'); return; }
    if (pin !== confirmPin) { setError('الرمزان غير متطابقين'); return; }

    setLoading(true);
    try {
      const result = await createVault({ name: name.trim(), type: VaultType.PERSONAL, pin });
      if (result.success) {
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'android' ? undefined : 'padding'} style={{ flex: 1 }}>
          <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} bounces={false}>
            <View style={styles.header}>
              <Typography variant="headlineMedium">إنشاء الخزنة</Typography>
              <Typography variant="bodyLarge" color={colors.onSurfaceVariant}>أنشئ خزنتك الآمنة لحماية بياناتك</Typography>
            </View>

            <Input label="اسم الخزنة" value={name} onChangeText={setName} placeholder="خزنتي الشخصية" autoCapitalize="words" returnKeyType="next" />

            <Typography variant="titleSmall" color={colors.onSurfaceVariant} style={styles.sectionLabel}>اختر أيقونة</Typography>
            <View style={styles.iconGrid}>
              {ICONS.map((icon) => (
                <TouchableOpacity
                  ...
                  accessibilityLabel={`أيقونة ${icon}`}
                >
            ...
            <Typography variant="titleSmall" color={colors.onSurfaceVariant} style={styles.sectionLabel}>اختر لوناً</Typography>
            ...
            <Input
              ...
              secureTextEntry
              maxLength={8}
              error={confirmPin.length > 0 && pin !== confirmPin ? 'الرقمان غير متطابقين' : undefined}
              returnKeyType="done"
            ...
            <Button
              title={loading ? 'جاري إنشاء الخزنة...' : 'إنشاء الخزنة'}
